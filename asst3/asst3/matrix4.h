...
inline Matrix4 transFact(const Matrix4& m) {
  ...
  // initialize it as an identity matrix
  Matrix4 r;
  // check the last row [0, 0, 0, 1]?
  assert(isAffine(m));
  // only copy the last column except the fourth row in that column
  for(int i = 0;i < 3;i++){
    r(i, 3) = m(i, 3);
  }
  return r;
...
inline Matrix4 linFact(const Matrix4& m) {
  ...
  // check the last row [0, 0, 0, 1]?
  assert(isAffine(m));
  // initialize it as the matrix given
  Matrix4 r = m;
  // name the last column 0 expect the last row
  r(0, 3) = r(1, 3) = r(2, 3) = 0;
  return r;
}
...
// return (O)t(E)r
// ---------------------------------------------
inline Matrix4 makeMixedFrame(const Matrix4& obj, const Matrix4& eye){
  assert(isAffine(obj));
  assert(isAffine(eye));
  // multiply transforlation of O and linearTrans of E.
  Matrix4 r = transFact(obj)*linFact(eye);
  return r;
...
// --------------------------------------------
inline Matrix4 doQtoOwrtA(const Matrix4& q, const Matrix4& o, const Matrix4& a){
  assert(isAffine(q));
  assert(isAffine(o));
  assert(isAffine(a));
  // just calculate...
  Matrix4 r = a * q * inv(a) * o;
  return r;
}
