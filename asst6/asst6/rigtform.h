...
inline RigTForm lerp(const RigTForm& tform0, const RigTForm& tform1, const double alpha){
  Cvec3 lerpTranslation = lerp(tform0.getTranslation(), tform1.getTranslation(), alpha);
  Quat lerpRotation = slerp(tform0.getRotation(), tform1.getRotation(), alpha);
  return RigTForm(lerpTranslation, lerpRotation);
}


inline RigTForm interpolateCatmullRom(
                          const RigTForm& tform0,
                          const RigTForm& tform1,
                          const RigTForm& tform2,
                          const RigTForm& tform3,
                          const double alpha){

  Cvec3 catmullRomTranslation = interpolateCatmullRom(tform0.getTranslation(),
                                                      tform1.getTranslation(),
                                                      tform2.getTranslation(),
                                                      tform3.getTranslation(), alpha);

  Quat catmullRomRotation = interpolateCatmullRom(tform0.getRotation(),
                                                  tform1.getRotation(),
                                                  tform2.getRotation(),
                                                  tform3.getRotation(),alpha);

  return RigTForm(catmullRomTranslation, catmullRomRotation);
