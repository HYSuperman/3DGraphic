...
bool Picker::visit(SgTransformNode& node) {
  ...
  // hw4 
  // cast node to a shared_ptr<SgNode> and add it to the stack
  shared_ptr<SgNode> nodeSg = node.shared_from_this();
  nodeStack_.push_back(nodeSg);
...
bool Picker::postVisit(SgTransformNode& node) {
  // hw4
  // after visiting a transform node
...
bool Picker::visit(SgShapeNode& node) {
  // hw4
  // Increase the idCounter_
  idCounter_++;
  // Find the SgRbtNode that is closest from the top of the stack
  shared_ptr<SgRbtNode> temp = dynamic_pointer_cast<SgRbtNode>(nodeStack_.back());
  // Add association between the idCounter_ and SgRbtNode
  if(temp){
    addToMap(idCounter_, temp);
  }
  else{
    return false;
  }
  ...
  // hw4
  // Convert the idCounter_ to RGB color, 
  // and set it to uIdColor through the handle h_uIdColor in ShaderState
  const Cvec3 tempColor = idToColor(idCounter_);
  safe_glUniform3f(drawer_.getCurSS().h_uIdColor, tempColor[0], tempColor[1], tempColor[2]);
...
shared_ptr<SgRbtNode> Picker::getRbtNodeAtXY(int x, int y) {
  ...
  // Read back a pixel from the framebuffer
  // and pack into a pixel
  PackedPixel p;
  glReadPixels(x, y, 1, 1,  GL_RGB, GL_UNSIGNED_BYTE, &p);

  // convert it into a ID
  int id = colorToId(p);
  // find it in the map
  shared_ptr<SgRbtNode> r = find(id);

  if(!r){
    return shared_ptr<SgRbtNode>();
  }
  else{
    return r;
  }
